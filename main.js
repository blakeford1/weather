...
$(document).on('click', '.pastSearch',  function(){
    ...
    function firstCitySearch (searchCity){
        $.ajax({
            url: "https://api.openweathermap.org/data/2.5/weather?q="+searchCity+"&units=imperial&appid=94a9f51f96f39e938bcacd6bd7b28c53", success: 
            function(result){
              $("#weatherdisplay").empty()
              
              var nameDate = $("<h3>").text(result.name + moment().format("   MMM Do YYYY"))
...
function fiveDayForecast () {
        ...
        for (let i = 0; i < forecast.length; i++) {
            ...
            var container =$('<div>')
            .attr("class", "fiveDayContainer")

            var iconUrl = 'http://openweathermap.org/img/wn/'+forecast[i].weather[0].icon+'@2x.png'
            
            var displayForecastDate = $("<h3>").text(moment(forecast[i].dt_txt.split(" ")[0]).format("MMM Do YY"))

            console.log(moment(forecast[i].dt_txt.split(" ")[0]).format("MMM Do YY"));
           ...
           // $("#forecastdisplay").append(displayForecastDate)
            ...
            container.append(displayForecastDate,iconPic, forecastTemp, forecastHumid)
